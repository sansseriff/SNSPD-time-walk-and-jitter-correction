...
def runAnalysisJit(path_, file_, modu_params, DERIV, PROP, delayScan = False, delay = 0, Figures = True):
    ...
    # plt.figure()
    # clocks = timetags[channels==clock_channel]
    # plt.plot(np.diff(clocks[:30000]), color = 'red')
    # plt.title("regular timetags")
    ...
    if Figures:
        ...
        # plt.figure()
        # plt.plot(np.diff(Clocks[:30000]))
        # plt.title("diff of clocks")

        # plt.figure()
        # plt.plot(np.diff(RecoveredClocks[:30000]))
        # plt.title("diff of recovered clocks")

    # if Figures:
    #     plt.figure()
    #     # checkLocking(dataTags[:20000]-nearestPulseTimes[:20000],dataTags[:20000]-nearestPulseTimes[:20000])
    #     plt.plot(np.arange(20000),dataTags[:20000]-nearestPulseTimes[:20000])
    #     plt.title("check dist")
    ...
    adjustment = [(i**2.9)*.00053 for i in range(20)]
    adjustment.reverse()
    st = 6
...
if __name__ == '__main__':
    ...
    if dBScan:
        ...
        # path = "..//data//537.5MHz_0.1s//"
        path = "..//data//537.5MHz//"
        file = "jitterate_4s_-0.049_537.5MHz_28.0.2.ttbin"
        ...
        dic = runAnalysisJit(path, file, modu_params, DERIV=500, PROP=5e-12, delayScan=False, delay=-191, Figures=True)
        print(dic)
        # 230
